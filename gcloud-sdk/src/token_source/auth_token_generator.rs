...
pub struct GoogleAuthTokenGenerator {
    ...
    cached_id_token: Arc<RwLock<Option<Token>>>,
}
...
impl GoogleAuthTokenGenerator {
    pub async fn new(
        ...
        Ok(GoogleAuthTokenGenerator {
            ...
            cached_id_token: Arc::new(RwLock::new(None)),
        })
    }
    ...
    pub async fn clear_cache(&self) {
        ...
        *write_state = None;
        let mut write_state = self.cached_id_token.write().await;
        *write_state = None;
    }
    ...
    pub async fn create_id_token(&self) -> crate::error::Result<Token> {
        let existing_token: Option<Token> = {
            let read_state = self.cached_id_token.read().await;
            read_state.clone()
        };
        ...
        match existing_token {
            ...
            _ => {
                let new_token = {
                    let mut write_token = self.cached_id_token.write().await;
                    ...
                    match write_token.as_ref() {
                        ...
                        _ => {
                            let new_token = self.token_source.identity().await?;
                            *write_token = Some(new_token.clone());
                            debug!(
                                "Created a new Google OAuth token. Expiring: {}. Source: {:?}",
                                new_token.expiry, self.token_source_type
